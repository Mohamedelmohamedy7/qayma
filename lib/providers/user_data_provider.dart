      ...
      await db
        ...
        'cart': {
          map['skuId']: {
            ...
            'skuId': map['skuId'],
            'skuName': map['skuName'],
            "payImageForUpload":map["payImageForUpload"],
            "productImageForUpload":map["productImageForUpload"],
            "priceDate":map["priceDate"],
            "DateOfProduct":map["DateOfProduct"]
          }
        },
      }, SetOptions(merge: true));
        ...
        // if (tempProd.isListed) {
          ...
          cartProducts.add(
            Cart.fromFirestore(
              ...
              item['quantity'],
              sku,
              item["payImageForUpload"],
              item["productImageForUpload"],
              item["priceDate"],
              item["skuName"],
              item["DateOfProduct"]
            ),
  ...
  List<Product> getNewSearch(String searchWord, List<Product> productsList) {
    try {
      ...
      for (var product in productsList) {
        if (product.name.toLowerCase().contains(searchWord)&&product.additionalInfo.bestBefore!=""&&product.additionalInfo.brand!=""&&product.additionalInfo.manufactureDate!=""&&product.additionalInfo.shelfLife!="") {
          print("انا هناgetNewSearch  ");
...
//TODO: add the new order values to adminInfo
  ...
  Future<bool> addAddress(
      String uid, List<Addresscust> address, int defaultAddress) async {
    print(address);
    ...
    for (var add in address) {
      ...
      tempAdd.putIfAbsent('lat', () => add.lat);
      tempAdd.putIfAbsent('long', () => add.log);
  ...
  Future<bool> removeAddress(
      String uid, List<Addresscust> address, bool isDefault) async {
    List<Map> addresses = List();
